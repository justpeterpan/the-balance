...
export default defineEventHandler(async (event) => {
  try {
    ...
    const tagRows = await db.select({ tags: bookmarksTable.tags }).from(bookmarksTable).where(eq(bookmarksTable.userId, event.context.user?.id ?? ''))
    const uniqueTags = new Set<string>()
    tagRows.forEach((row) => { if(row.tags) {
